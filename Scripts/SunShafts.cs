...
public class SunShafts : PostEffectsBase
{
    ...
    private Shader simpleClearShader;
    private Material simpleClearMaterial;

#if UNITY_EDITOR
    public static SunShafts editorSunShafts = null;
    public bool syncWithEditorCamera = true;
#endif
    ...
    public override void Start()
    {
        ...
        {
            Debug.LogError("Error starting sunshafts effect, couldn't find the shader: Hidden/SunShaftsComposite");
            Destroy(this);
        }
        if (simpleClearShader == null)
        {
            Debug.LogError("Error starting sunshafts effect, couldn't find the shader: Hidden/SimpleClear");
            Destroy(this);
        }
        base.Start();
    }
    ...
    {
#if UNITY_EDITOR
        if (syncWithEditorCamera)
        {
            if (editorSunShafts != null)
            {
                editorSunShafts.enabled = true;
            }
    ...
    private void EditorUpdate()
    {
        if (syncWithEditorCamera)
        {
            if (editorSunShafts == null)
            {
                ...
                {
                    if (Camera.current.gameObject.GetComponent<SunShafts>())
                    {
                        editorSunShafts = Camera.current.gameObject.GetComponent<SunShafts>();
                    }
                    else
                    {
                        editorSunShafts = Camera.current.gameObject.AddComponent<SunShafts>();
                        editorSunShafts.syncWithEditorCamera = false;
                    }
                    SyncSunShaftsParamatersWithEditor();
                }
            }
            else
            {
    ...
    private void SyncSunShaftsParamatersWithEditor()
    {
        editorSunShafts.maxRadius = maxRadius;
        editorSunShafts.radialBlurIterations = radialBlurIterations;
        editorSunShafts.resolution = resolution;
        editorSunShafts.screenBlendMode = screenBlendMode;
        editorSunShafts.sunColor = sunColor;
        editorSunShafts.sunShaftBlurRadius = sunShaftBlurRadius;
        editorSunShafts.sunShaftIntensity = sunShaftIntensity;
        editorSunShafts.sunThreshold = sunThreshold;
        editorSunShafts.useDepthTexture = useDepthTexture;
        editorSunShafts.sunTransform = sunTransform;
    }
#endif
